 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "    target_dataset_dir = os.path.join(DRIVE_PATHS[\"root\"], \"datasets\", \"02_character\")\n",
    "\n",
    "# Tự động phát hiện thư mục ảnh (kể cả khi để trong thư mục con, sâu nhiều tầng, hoặc file zip)\n",
    "target_dataset_dir = DatasetNormalizer.resolve_dataset_dir(target_dataset_dir)\n",
    "os.makedirs(target_dataset_dir, exist_ok=True)\n",
    "\n",
    "valid_exts = {\".png\", \".jpg\", \".jpeg\", \".webp\", \".bmp\", \".jfif\", \".tiff\", \".heic\", \".heif\", \".avif\"}\n",
    "all_files_before = os.listdir(target_dataset_dir) if os.path.exists(target_dataset_dir) else []\n",
    "raw_img_files = [f for f in all_files_before if os.path.splitext(f)[-1].lower() in valid_exts]\n",
    "\n",
    "if len(raw_img_files) == 0:\n",
    "    print(f\"\\n❌ [CẢNH BÁO] KHÔNG TÌM THẤY ẢNH HỢP LỆ NÀO TẠI:\\n   📂 {target_dataset_dir}\")\n",
    "    print(f\"\\n👉 Hướng dẫn khắc phục:\")\n",
    "    print(f\"   1. Mở Google Drive của bạn, kiểm tra xem ảnh đã được tải lên thư mục trên chưa.\")\n",
    "    print(f\"   2. Nếu bạn để ảnh trong thư mục riêng (vd: MyDrive/anh_cua_toi), hãy chọn '06_custom_path' ở ô DATASET_CATEGORY và dán đường dẫn vào ô CUSTOM_DATASET_PATH.\")\n",
    "    print(f\"   3. Nếu bạn tải lên file .zip, hệ thống sẽ tự động giải nén khi bạn chạy lại Cell 4.\")\n",
    "    \n",
    "    # Quét toàn bộ thư mục datasets trên Google Drive để tìm ảnh và gợi ý vị trí chính xác\n",
    "    datasets_root = os.path.join(DRIVE_PATHS[\"root\"], \"datasets\")\n",
    "    if os.path.exists(datasets_root):\n",
    "        found_suggestions = []\n",
    "        for root_d, dirs_d, f_list in os.walk(datasets_root):\n",
    "            dirs_d[:] = [d for d in dirs_d if not d.startswith((\".\", \"_\"))]\n",
    "            cnt = len([f for f in f_list if os.path.splitext(f)[-1].lower() in valid_exts])\n",
    "            if cnt > 0:\n",
    "                found_suggestions.append((root_d, cnt))\n",
    "        if found_suggestions:\n",
    "            print(f\"\\n💡 TÌM THẤY ẢNH Ở CÁC THƯ MỤC KHÁC TRÊN GOOGLE DRIVE:\")\n",
    "            for s_dir, s_cnt in found_suggestions:\n",
    "                print(f\"   • {s_dir} ({s_cnt} ảnh)\")\n",
    "else:\n",
    ...
    "    target_dataset_dir = norm_result.get(\"target_dir\", target_dataset_dir)\n",
    "    \n",
    ...
    "    \n",
    "    print(f\"\\n✅ Dữ liệu ({len(raw_img_files)} ảnh) tại '{target_dataset_dir}' đã sẵn sàng 100% để train!\")"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "#@markdown * **Training Engine**: Tự động chọn engine tối ưu nhất (`sd-scripts` cho SDXL/Pony/SD1.5/Flux, `AI-Toolkit` cho SDXL/Krea, `musubi-tuner` cho Wan/Qwen).\n",
    ...
    "\n",
    "TRAINING_ENGINE = \"auto (Tu Dong Chon Engine Toi Uu Theo Model)\" #@param [\"auto (Tu Dong Chon Engine Toi Uu Theo Model)\", \"sd-scripts (Kohya_ss SDXL/Pony/SD1.5/Flux)\", \"ai-toolkit (Ostris SDXL/Krea)\", \"musubi-tuner (Wan/Qwen)\", \"diffusers\"]\n",
    ...
    "    opt_type = \"AdamW\"\n",
    "\n",
    "resolved_precision = CURRENT_PROFILE.precision if 'CURRENT_PROFILE' in locals() and CURRENT_PROFILE else 'fp16'\n",
    "effective_sample_prompt = SAMPLE_PROMPT\n",
    "if 'TRIGGER_WORD' in locals() and TRIGGER_WORD.strip() and TRIGGER_WORD.strip() != 'sks person':\n",
    "    if 'sks person' in effective_sample_prompt:\n",
    "        effective_sample_prompt = effective_sample_prompt.replace('sks person', TRIGGER_WORD.strip())\n",
    "    elif not any(tw in effective_sample_prompt for tw in [TRIGGER_WORD.strip()]):\n",
    "        effective_sample_prompt = f\"{TRIGGER_WORD.strip()}, {effective_sample_prompt}\"\n",
    "\n",
    ...
    "        sample_prompt=effective_sample_prompt\n",
    "    )\n",
    ")\n",
    "\n",
    ...
    "print(f\"\\n✅ Cell 5: Cấu hình ({opt_type} | LR: {resolved_lr} | Precision: {resolved_precision} | Batch: {BATCH_SIZE} | Epochs: {EPOCHS}) đã lưu tại Google Drive:\")\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "trainer = EngineFactory.create_trainer(active_config, explicit_choice=TRAINING_ENGINE)\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "from lora_colab.storage.resume_manager import ResumeManager\n",
    "\n",
    "expected_lora = os.path.join(active_config.training.output_dir, f\"{active_config.training.output_name}.safetensors\")\n",
    "latest_lora = None\n",
    "if os.path.exists(expected_lora):\n",
    "    latest_lora = expected_lora\n",
    "else:\n",
    "    ckpt_status = ResumeManager.get_resume_status(active_config.training.checkpoint_dir)\n",
    "    if ckpt_status[\"can_resume\"]:\n",
    "        latest_lora = ckpt_status[\"checkpoint_path\"]\n",
    "\n",
    "if latest_lora:\n",
    "    print(f\"🎯 Đã phát hiện checkpoint LoRA: {latest_lora}\")\n",
    "else:\n",
    "    print(f\"ℹ️ Chưa có file LoRA nào trong outputs/checkpoints. Test preview sẽ dùng model nền gốc.\")\n",
    "\n",
    "effective_test_prompt = TEST_PROMPT\n",
    "if 'TRIGGER_WORD' in locals() and TRIGGER_WORD.strip() and TRIGGER_WORD.strip() != 'sks person':\n",
    "    if 'sks person' in effective_test_prompt:\n",
    "        effective_test_prompt = effective_test_prompt.replace('sks person', TRIGGER_WORD.strip())\n",
    "    elif not any(tw in effective_test_prompt for tw in [TRIGGER_WORD.strip()]):\n",
    "        effective_test_prompt = f\"{TRIGGER_WORD.strip()}, {effective_test_prompt}\"\n",
    "\n",
    ...
    "        base_model_path=model_file_path,\n",
    "        lora_weights_path=latest_lora,\n",
    "        prompt=effective_test_prompt,\n",
