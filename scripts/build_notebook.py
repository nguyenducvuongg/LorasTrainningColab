 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "import sys\n",
    "\n",
    "# 1. Clone / Pull Repository từ GitHub\n",
    "REPO_URL = \"https://github.com/nguyenducvuongg/LorasTrainning.git\" #@param {type:\"string\"}\n",
    "WORKSPACE_DIR = \"/content/LorasTrainning\"\n",
    "\n",
    "if not os.path.exists(WORKSPACE_DIR):\n",
    "    print(f\"📦 Cloning repository from {REPO_URL}...\")\n",
    "    !git clone {REPO_URL} {WORKSPACE_DIR}\n",
    "else:\n",
    "    print(f\"🔄 Updating repository in {WORKSPACE_DIR}...\")\n",
    ...
    "\n",
    "# 2. Tự Động Nhận Diện & Tối Ưu Môi Trường Colab Động (Không Xung Đột Phiên Bản)\n",
    "from lora_colab.core.environment import AutoEnvironmentManager\n",
    "AutoEnvironmentManager.optimize_and_install_dependencies()\n",
    "\n",
    "# 3. Mount Google Drive & Khởi tạo cây thư mục chuẩn (Chống ghi đè)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "CAPTION_ENGINE = \"gemini (Google Cloud Vision - 0% VRAM)\" #@param [\"gemini (Google Cloud Vision - 0% VRAM)\", \"florence2 (Florence-2 / JoyCaption Local VLM)\", \"deepseek (Cloud API OpenAI format)\", \"wd14 (Anime / Danbooru tags - Local)\", \"skip_captioning (Da co san file .txt)\"]\n",
    ...
    "\n",
    "# 1. Chuẩn hoá ảnh sang RGB & Đổi tên đồng bộ {prefix}_{index:04d}\n",
    "norm_result = DatasetNormalizer.normalize_folder(target_dataset_dir, prefix=FILENAME_PREFIX)\n",
    "\n",
    "# 2. Gán nhãn tự động nếu được chọn\n",
    "if \"skip_captioning\" not in CAPTION_ENGINE:\n",
    ...
    "    elif \"florence2\" in CAPTION_ENGINE or \"joycaption\" in CAPTION_ENGINE:\n",
    "        captioner = Florence2Captioner(model_name=\"florence-2-base\", task=\"more_detailed\", cache_dir=caption_cache)\n",
    "    elif \"wd14\" in CAPTION_ENGINE:\n",
    "        captioner = WD14Tagger()\n",
    "    else:\n",
    "        captioner = Florence2Captioner(cache_dir=caption_cache)\n",
    "        \n",
    "    captioner.caption_directory(target_dataset_dir, trigger_word=TRIGGER_WORD)\n",
    "\n",
    "print(f\"\\n✅ Dữ liệu tại '{target_dataset_dir}' đã sẵn sàng để train!\")"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "from lora_colab.engines.diffusers_trainer import DiffusersTrainer\n",
    "\n",
    "resume_path = None\n",
    ...
    "\n",
    "fam = active_config.training.model_family.lower()\n",
    "if \"flux\" in fam or \"krea\" in fam:\n",
    "    trainer = AIToolkitTrainer(active_config)\n",
    "elif any(k in fam for k in [\"sdxl\", \"pony\", \"illustrious\", \"sd35\", \"sd15\"]):\n",
    "    trainer = KohyaTrainer(active_config)\n",
    "else:\n",
    "    trainer = DiffusersTrainer(active_config)\n",
    "\n",
    "print(f\"🚀 Bắt đầu quá trình huấn luyện LoRA cho: {active_config.training.model_family}...\")\n",
